...
const StreamingCard: FC<{
  mode: string;
  _streamType: string;
  txData?: IDetailBreakdownSingle | IDetailBreakdownMulti;
}> = ({ mode, _streamType, txData }) => {
  ...
  const updateDetailData = (changedVal: any, allVal: any) => {
    ...
    if (allVal.token) setDetailData(allVal);
  ...
  const handleSubmit = async (values: any) => {
    ...
    const UnitsPerSecond = parseEther(values.flowrate.value).div(
      timeInSeconds[values.flowrate.unit]
    ); //format if unit is more than second
    const parsedDeposit = parseEther("1");
    await Account.execute([
      {
        contractAddress: contracts[values.token]
          ? contracts[values.token].address
          : "",
                    ...
                    validator: async (_, names) => {
                      if (!names || names.length < 2) {
                        return Promise.reject(
                          new Error("At least 2 receivers")
                        );
                      }
                    },
                  },
                  ...
                  <>
                    {fields.map(({ key, name, ...restField }, index) => (
                      <Space
                        ...
                        align="center"
                      >
                        <Form.Item required={false} name={[name, "addr"]}>
                          <StyledInput
                            style={{ backgroundColor: "#2d1857" }}
                            disabled={disableEdit}
                            allowClear
